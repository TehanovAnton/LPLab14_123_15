...
{ 
	...
	void PNLTstr::AddlexWithITcorection(LT::Entry entryL, IT::IdTable& idTable, int indx)
	{
		this->pnstr[indx] = entryL;
		idTable.table[entryL.idxTI].idxfirstLE = indx;
	}
	...
	//		{
	//			// запись операнда в порядке следования
	//			res.AddlexWithITcorection(lexLT, idTable, res.size++);
	//			parmCounter++;
	//		}
	//		// функция
	...
	//				{					
	//					res.AddlexWithITcorection(stck.top(), idTable, res.size++);
	//					stck.pop();
	//				}
	//				stck.pop();
	//			}
	...
	//	_itoa_s(parmCounter, parCou, 10);
	//	// pC - parmCounter
	//	LT::Entry pC = LT::Entry();
	//	pC.lexema[0] = parCou[0];
	//	res.AddlexWithITcorection(pC, idTable, res.size++);
	//	res.AddlexWithITcorection(func, idTable, res.size++);
	//}
